...
### P4 — SPA cutover (P4a ✅ DONE) + delete dead TS run logic (P4b ← NEXT)

**P4a (cutover) is DONE this session** (`30dbb7214` + `ceaeac078`) — see the "Done" list above. **P4b (deletion) is next, and its scope is CORRECTED from the original plan below** — a 24-agent value-import blast-radius audit (run `wf_dcc89776-4d0`) found the prompt's "delete all of `shared/src/runs/*`" is wrong:

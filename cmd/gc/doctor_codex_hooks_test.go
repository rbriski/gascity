...
func TestCodexHooksDriftCheckPassesCurrentHooks(t *testing.T) {
  ...
  "hooks": {
    "SessionStart": [{
      "hooks": [{
        "type": "command",
        "command": "export PATH=\"$HOME/go/bin:$HOME/.local/bin:$PATH\" && if command -v timeout >/dev/null 2>&1; then _gcbd_skew=$(timeout 10 bd doctor 2>&1); elif command -v gtimeout >/dev/null 2>&1; then _gcbd_skew=$(gtimeout 10 bd doctor 2>&1); else _gcbd_skew=$(bd doctor 2>&1); fi; case \"$_gcbd_skew\" in *\"schema version mismatch\"*|*\"Unable to open database\"*) printf '%%s\\n' \"$_gcbd_skew\" >&2; exit 1 ;; esac; GC_MANAGED_SESSION_HOOK=1 GC_HOOK_EVENT_NAME=SessionStart gc --city %s prime --hook --hook-format codex"
      }]
    }],
    "PreCompact": [{
      "hooks": [{

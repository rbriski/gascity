...
### Phase 2 — remote client transport (gate G6)
- NEW `internal/api/client_remote.go`: `NewRemoteCityScopedClient(baseURL, cityName, opts…)` — sets `isRemote`; **one TLS/redirect policy → two `*http.Client` shapes**: REST (`Timeout:120s` + dial/TLS sub-budgets) and STREAM (`Timeout:0` + per-frame-reset context-cancel idle watchdog ~45s; **never** `http.Client.Timeout` on a stream). `CheckRedirect` **strips `Authorization` + every `X-GC-*` and refuses cross-host + refuses https→http downgrade**. Thread the stream shape + a **mutex-guarded token source** + `CheckRedirect` into **`waitForEvent` (`client.go:322`) AND `eventsAPIScope.client()` (`cmd_events.go:131`)** — both bare `http.Client` today. Add the `Authorization` bearer editor alongside the existing `X-GC-Request` editor in `newClient`/the remote constructor.

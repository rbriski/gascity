...
- [x] **B1a** **Router forwards tier-selected creates/pours by class** (Option B — the Router sits *below* `beadPolicyStore`, not replacing it, so the config-coupled policy logic stays in `cmd/gc` and `internal/coordrouter` stays config-free). `createWithStoragePolicy` already calls `CreateWithStorage` on its wrapped store, so it suffices for the Router to implement `StorageCreateStore` (`Router.CreateWithStorage` routes by class) and `StorageGraphApplyStore` (`routedGraphApplier.ApplyGraphPlanWithStorage` routes by class + forwards the tier); plus `SQLiteStore.CreateWithStorage`. `beadPolicyStore` **keeps** tier-selection + read-tier expansion + `Count`/`Handles`/`ReleaseIfCurrent` (above the Router). This is the elegant resolution of the red-team's "Router drops tier/policy" — the tier flows *down* through the Router rather than being ported *up* into it. *Tests (green): Router routes a tiered create + a tiered pour to the owning backend carrying the tier; SQLite `CreateWithStorage` round-trips ephemeral/no_history.*
- [ ] **B1b** Construct `beadPolicyStore(Router(per-class CachingStore(backend)))` in `api_state.go` — `wrapStoreWithBeadPolicies` still returns `*beadPolicyStore`/`*beadPolicyGraphStore` (wrapping the Router), so `unwrapBeadPolicyStore` already matches; the change is the construction (build the Router with cached per-class backends, then wrap in policy) + the re-wrap path. *Test: full suite + a byte-identical differential test running the FULL CachingStore-inside / Router-outside re-wrap, canonicalizing & comparing ids + edges + **tiers** + wire JSON of a graph.v2 pour. In the identity phase (all classes → one bd) the Router is a pure passthrough, so the embedded-primary delegation for reads/Count/Handles/ReleaseIfCurrent is byte-identical; federation (B2) and per-id routing (B3) only bite once a class points at a different backend.*

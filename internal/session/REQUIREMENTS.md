...
| SESSION-RECON-012 | Inert transport-failure recovery | A desired/live session left alive but inert after a provider transport failure (Codex WebSocket→HTTPS fallback timeout or HTTPS stream disconnect, or a DNS lookup failure such as Claude's ENOTFOUND) is recovered by a bounded continuation nudge into the same session (preserving its conversation/work context), never a restart. Detection requires BOTH a high-confidence transport-failure signature AND an idle prompt within the recent pane tail, so ordinary idle prompts and active turns are never recovered, and historical/pasted error text cannot false-trigger. Recovery is provider-neutral and role-neutral: eligibility is orchestrator-desired (assigned work is sufficient but not necessary), and attachment is NOT an exemption (the incident's crashed session was attached). The observe→nudge→backoff→give-up state machine is persisted on the session bead (`inert_recovery_fingerprint`/`inert_recovery_count`/`inert_recovery_at`) so a controller restart cannot replay it; the attempt count persists even when a nudge errors; peeks are bounded by a quiet gate plus an in-memory activity checkpoint (one completed turn → at most one inspection); nudges are capped per episode and deduped by failure fingerprint so recovery cannot form a restart storm. | `internal/runtime/transport_failure.go` (`ClassifyInertTransportFailure`); `internal/runtime/transport_failure_test.go`; `internal/session/lifecycle_recovery.go` (`DecideInertRecovery`, `InertRecovery*Patch`); `internal/session/lifecycle_recovery_test.go`; `cmd/gc/inert_recovery.go` (`recoverInertSessions`); `cmd/gc/inert_recovery_test.go`; incident ci-emg / bead ga-qox |

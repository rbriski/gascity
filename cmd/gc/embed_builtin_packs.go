...
func EnsureBuiltinRuntimeAssets(cityPath string, warningWriter io.Writer) error {
	...
	if state.ready && requiredBuiltinSourcesUsable(cityPath) && lockedBundledImportsUsable(cityPath) && gcBeadsBdShimUsable(cityPath) {
		return nil
	}
	state.ready = false
...
// gcBeadsBdShimContent returns the exact bytes the stable gc-beads-bd shim
// must contain for the running binary: a POSIX entrypoint execing the
// cache-resolved bundled bd lifecycle script. It is the single source of
// truth shared by the writer (ensureGcBeadsBdShim) and the ready-fast-path
// validator (gcBeadsBdShimUsable), so a "usable" shim is defined as one whose
// on-disk bytes match exactly what the writer would produce now.
func gcBeadsBdShimContent() ([]byte, error) {
...
func ensureGcBeadsBdShim(cityPath string) error {
	...
	return fsys.WriteFileIfContentOrModeChangedAtomic(fsys.OSFS{}, path, shim, 0o755)
}

// gcBeadsBdShimUsable reports whether the on-disk gc-beads-bd shim already
// projects the running binary's bundled target. Cities that do not use the
// bd store contract need no shim and are trivially usable.
//
// The ready fast path in EnsureBuiltinRuntimeAssets validates the live bd
// cache slot (requiredBuiltinSourcesUsable) but not the shim itself. A shim
// execing a *different* target — an older binary's since-evicted cache slot,
// or a foreign temp path a test wrote when its GC_HOME resolved to this city
// — therefore slips past the guard and is never rewritten. Scheduled beads
// health and recovery exec the shim directly, so a stale or vanished target
// silently breaks them (ga-a76). Reporting a mismatched (or missing/unreadable)
// shim as unusable forces the fast path to fall through and regenerate it.
func gcBeadsBdShimUsable(cityPath string) bool {
	if !cityUsesBdStoreContract(cityPath) {
		return true
	}
	want, err := gcBeadsBdShimContent()
	if err != nil {
		return false
	}
	got, err := os.ReadFile(gcBeadsBdScriptPath(cityPath))
	if err != nil {
		return false
	}

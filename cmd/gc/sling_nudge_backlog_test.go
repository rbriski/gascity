...
// backlogSlowStore stands in for a contended Dolt backend: every store
// operation sleeps for latency before returning a fixed, valid response.
...
// backlog. Before nudgeEnqueueMaintenanceBudget this ran O(backlog) serial
// store ops under the withNudgeQueueState flock with no aggregate deadline.

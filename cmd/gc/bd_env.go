...
func mirrorBeadsDoltEnv(env map[string]string) {
	...
	// BEADS_DOLT_CREDENTIAL_COMMAND (beads configfile.GetDoltCredentialCommand);
	// without it bd falls back to the static/root user and the gateway rejects
	// the connection with "access denied" (Error 1045). That key contains the
	// substring CREDENTIAL, so execenv.FilterInherited strips it from every
	// gc-spawned subprocess and agent session — the in-process native store
	// still authenticates (it reads the un-filtered ambient env), but shell-out
	// bd and spawned agents do not. The controller image also exports the same
	// helper path under the non-sensitive GC_DOLT_CRED_CMD, which survives
	// filtering, so mirror it back into BEADS_DOLT_CREDENTIAL_COMMAND here
	// (map/GC_DOLT_CRED_CMD wins, else the ambient value of either key) so bd
	// authenticates the same way the native store does.

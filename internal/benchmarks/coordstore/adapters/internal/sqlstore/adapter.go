...
func (a *Adapter) SetMetadataBatch(ctx context.Context, id string, kvs map[string]string) error {
	...
	if _, err := tx.ExecContext(ctx,
		...
		return fmt.Errorf("%s SetMetadataBatch timestamp: %w", a.dialect.Name, err)
	}
	return tx.Commit()
}
...
func (a *Adapter) PurgeTerminal(ctx context.Context, olderThan time.Duration) (int, error) {
	...
	rows, err := a.db.QueryContext(ctx,
		"SELECT id FROM records WHERE status IN ("+
			...
			") AND COALESCE(NULLIF(updated_at, 0), created_at) < "+a.dialect.Placeholder(5),
		"closed", "canceled", "canceled", "expired", cutoff)
	if err != nil {

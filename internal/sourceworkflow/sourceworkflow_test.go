...
func TestCloseWorkflowGeneratedSpecsClosesOnlyOpenSpecs(t *testing.T) {
	store := beads.NewMemStore()
	root, err := store.Create(beads.Bead{
		Title:  "root",
		Type:   "task",
	...
	closed, err := CloseWorkflowGeneratedSpecs(store, root.ID)
	if err != nil {
		t.Fatalf("CloseWorkflowGeneratedSpecs: %v", err)
	}
	if closed != 1 {
		t.Fatalf("CloseWorkflowGeneratedSpecs closed %d beads, want 1", closed)
	}
	...
	if got := specAfter.Metadata["close_reason"]; got != WorkflowGeneratedSpecClosedReason {
		t.Fatalf("spec close_reason = %q, want %q", got, WorkflowGeneratedSpecClosedReason)
	}

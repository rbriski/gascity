...
/**
 * Admit a durable session nudge
 *
 * Authenticates requester evidence from the trusted request context and durably admits one idempotent session nudge without invoking the runtime provider.
 */
export const admitSessionNudge = <ThrowOnError extends boolean = false>(options: Options<AdmitSessionNudgeData, ThrowOnError>) => (options.client ?? client).post<AdmitSessionNudgeResponses, AdmitSessionNudgeErrors, ThrowOnError>({

...
func TestSweepStaleNudgeMail_TTLBoundaries(t *testing.T) {
	...
	result, err := sweepStaleNudgeMail(store, store, nil, now, nudgeTTL, mailTTL, 0)
	if err != nil {
...
func TestSweepStaleNudgeMail_PendingExclusion(t *testing.T) {
	...
	result, err := sweepStaleNudgeMail(store, store, state, now, nudgeTTL, time.Hour, 0)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.NudgeClosed != 1 {
...
func TestSweepStaleNudgeMail_OpenStatusFilter(t *testing.T) {
	...
	result, err := sweepStaleNudgeMail(store, store, nil, now, time.Minute, mailTTL, 0)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.MailClosed != 1 {
...
func TestSweepStaleNudgeMail_BudgetCap(t *testing.T) {
	...
	result, err := sweepStaleNudgeMail(store, store, nil, now, nudgeTTL, mailTTL, budget)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestSweepStaleNudgeMail_PerBeadCloseFailureContinues(t *testing.T) {
	...
	result, err := sweepStaleNudgeMail(store, store, nil, now, nudgeTTL, time.Hour, 0)
...
func TestSweepStaleNudgeMail_BudgetSplitNudgeThenMail(t *testing.T) {
	...
	result, err := sweepStaleNudgeMail(store, store, nil, now, nudgeTTL, mailTTL, 3)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result.NudgeClosed != 2 {
...
func TestCmdOrderSweepNudgeMailRun_NothingToClose(t *testing.T) {
	...
	cmdOrderSweepNudgeMailRun(store, store, nil, now, nudgeMailSweepDefaultNudgeTTL, nudgeMailSweepDefaultMailTTL, false, &stdout, &stderr)
...
func TestCmdOrderSweepNudgeMailRun_QuietSuppressesOutput(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	cmdOrderSweepNudgeMailRun(store, store, nil, now, nudgeMailSweepDefaultNudgeTTL, nudgeMailSweepDefaultMailTTL, true, &stdout, &stderr)
	if stdout.String() != "" {
...
func TestCmdOrderSweepNudgeMailDryRun_NothingToClose(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	cmdOrderSweepNudgeMailDryRun(store, store, nil, now, nudgeMailSweepDefaultNudgeTTL, nudgeMailSweepDefaultMailTTL, false, &stdout, &stderr)
	if !strings.Contains(stdout.String(), "nothing to close") {
...
func TestCmdOrderSweepNudgeMailDryRun_ListErrorReturnsNonZero(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	code := cmdOrderSweepNudgeMailDryRun(store, store, nil, now, nudgeMailSweepDefaultNudgeTTL, nudgeMailSweepDefaultMailTTL, false, &stdout, &stderr)
	if code == 0 {
...
func TestCmdOrderSweepNudgeMailRun_ListErrorReturnsNonZero(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	code := cmdOrderSweepNudgeMailRun(store, store, nil, now, nudgeMailSweepDefaultNudgeTTL, nudgeMailSweepDefaultMailTTL, false, &stdout, &stderr)
	if code == 0 {
		t.Errorf("expected non-zero exit code on list error, got %d", code)
	}
	if !strings.Contains(stderr.String(), "gc order sweep-nudge-mail:") {
...
func TestCountStaleNudgeMail_ListErrorPropagates(t *testing.T) {
	...
	_, err := countStaleNudgeMail(store, store, nil, now, nudgeMailSweepDefaultNudgeTTL, nudgeMailSweepDefaultMailTTL, 0)
	if err == nil {
...
func TestRunNudgeMailSweepWatchdog_ClosesStaleBeads(t *testing.T) {
	...
	result, err := sweepStaleNudgeMail(store, store, nil, now, nudgeMailSweepDefaultNudgeTTL, nudgeMailSweepDefaultMailTTL, nudgeMailSweepWatchdogCloseBudget)
	if err != nil {
...
func TestCountStaleNudgeMail_MatchesSweepCounts(t *testing.T) {
	...
	counts, err := countStaleNudgeMail(store, store, nil, now, nudgeTTL, mailTTL, 0)
	if err != nil {

...
// strandedPoolInstanceTestCfg returns a city with a single min=0 pool agent
// whose qualified name is "fixture/worker".
func strandedPoolInstanceTestCfg() *config.City {
	return &config.City{
		Workspace: config.Workspace{Name: "test-city"},
		Rigs:      []config.Rig{{Name: "fixture", Path: ""}},
		Agents: []config.Agent{{
...
func TestStrandedPoolInstanceDemand_CountsWorkOnGoneInstance(t *testing.T) {
	cfg := strandedPoolInstanceTestCfg()
	template := cfg.Agents[0].QualifiedName()
	// Open work routed to the pool template but stranded on a pool instance /
	// session name that has NO open session bead (snapshot is empty == complete).
	work := beads.Bead{
		ID:       "gcg-23482",
		Title:    "stranded group-2 step",
		Type:     "task",
		Status:   "open",
		Assignee: "worker-mc-wisp-cydc",
		Metadata: map[string]string{"gc.routed_to": template},
	}
	demand := strandedPoolInstanceDemand(cfg, []beads.Bead{work}, newSessionBeadSnapshot(nil))
	if !demand[template] {
		t.Fatalf("strandedPoolInstanceDemand = %#v, want demand for %q (gone instance must scale pool from zero)", demand, template)
	}
}

func TestStrandedPoolInstanceDemand_SkipsWorkOwnedByLiveInstance(t *testing.T) {
	cfg := strandedPoolInstanceTestCfg()
	template := cfg.Agents[0].QualifiedName()
	const sessionName = "worker-mc-wisp-cydc"
	...
	demand := strandedPoolInstanceDemand(cfg, []beads.Bead{work}, newSessionBeadSnapshot([]beads.Bead{liveSession}))
	if demand[template] {
		t.Fatalf("strandedPoolInstanceDemand = %#v, want no demand: a LIVE session owns the work and the resume tier handles it", demand)
	}
}

func TestStrandedPoolInstanceDemand_IgnoresUnassignedAndTemplateAssignee(t *testing.T) {
	cfg := strandedPoolInstanceTestCfg()
	template := cfg.Agents[0].QualifiedName()
	work := []beads.Bead{
		{
			ID:       "gcg-open-unassigned",
			Title:    "unassigned routed work",
			Type:     "task",
			Status:   "open",
			Metadata: map[string]string{"gc.routed_to": template},
		},
		{
			// Assignee IS the pool template (legacy bound / template form): the
			// wake-known-identity tier owns this; this pass must not double-count.
			ID:       "gcg-template-assignee",
			Title:    "template-assigned routed work",
			Type:     "task",
			Status:   "open",
			Assignee: template,
			Metadata: map[string]string{"gc.routed_to": template},
		...
		{
			// Stranded instance but routed to an UNKNOWN template: not our job.
			ID:       "gcg-unknown-route",
			Title:    "work for an unconfigured agent",
			Type:     "task",
			Status:   "open",
			Assignee: "ghost-mc-wisp-zzzz",
			Metadata: map[string]string{"gc.routed_to": "fixture/ghost"},
		},
	}
	demand := strandedPoolInstanceDemand(cfg, work, newSessionBeadSnapshot(nil))
	if len(demand) != 0 {
		t.Fatalf("strandedPoolInstanceDemand = %#v, want empty: unassigned, template-assigned, and unknown-route work must not contribute instance demand", demand)
	}
}

func TestBuildDesiredState_StrandedPoolInstance_NilSnapshotReportsDemand(t *testing.T) {
	cfg := strandedPoolInstanceTestCfg()
	template := cfg.Agents[0].QualifiedName()
	// strandedPoolInstanceDemand does NOT self-gate on a nil/degraded snapshot:
	// with an empty session index every assignee looks dead, so it reports demand.
	// The build call site — not this pure function — is what skips on nil/LoadError
	// (covered by the call-site tests below). Pin the pure function's contract here.
	demand := strandedPoolInstanceDemand(cfg, []beads.Bead{{
		ID:       "gcg-23482",
		Title:    "stranded",
		Type:     "task",
		Status:   "open",
	...
	if len(demand) != 1 {
		t.Fatalf("nil snapshot: demand = %#v, want exactly one template (empty index => assignee looks dead; the caller is what gates on nil)", demand)
	}
}

func TestBuildDesiredState_StrandedPoolInstance_RespawnsAndRehomes(t *testing.T) {
	cityPath := t.TempDir()
	rigPath := filepath.Join(cityPath, "fixture")
	if err := os.MkdirAll(rigPath, 0o755); err != nil {
		t.Fatalf("create rig dir: %v", err)
	...
	cityStore := beads.NewMemStore()
	rigStore := beads.NewMemStore()
	// Stranded group-boundary work: open, routed to the pool, assignee is a gone
	// pool instance whose session bead no longer exists.
	if _, err := rigStore.Create(beads.Bead{
		Title:    "stranded group-2 step",
		Type:     "task",
		Status:   "open",
	...
	dsResult := buildDesiredStateWithSessionBeads(
		cfg.EffectiveCityName(), cityPath, time.Now().UTC(), cfg,
		runtime.NewFake(), cityStore, map[string]beads.Store{"fixture": rigStore},
		snapshot, nil, &stderr,
	)

	if got := dsResult.ScaleCheckCounts[template]; got < 1 {
		t.Fatalf("ScaleCheckCounts[%s] = %d, want >=1: stranded pool-instance work must scale the pool from zero; stderr:\n%s", template, got, stderr.String())
	}
	sessions := 0
	for _, tp := range dsResult.State {
		if tp.TemplateName == template {
	...
	if sessions < 1 {
		t.Fatalf("desired sessions for %s = %d, want >=1 to claim the stranded work; stderr:\n%s", template, sessions, stderr.String())
	}

	// Re-home half: releaseOrphanedPoolAssignments runs on a complete snapshot and
	// clears the dead-instance assignee so the freshly-spawned member can claim it.
	open := snapshot.Open()
	released := releaseOrphanedPoolAssignmentsWhenSnapshotsComplete(cityStore, cfg, cityPath, open, dsResult, map[string]beads.Store{"fixture": rigStore})
	if len(released) != 1 {
		t.Fatalf("releaseOrphanedPoolAssignments released %d beads, want 1 (the stranded instance assignment); stderr:\n%s", len(released), stderr.String())
	}
	rehomed, err := rigStore.List(beads.ListQuery{Status: "open"})
	if err != nil {
		t.Fatalf("list rig work: %v", err)
	}
	if len(rehomed) != 1 {
		t.Fatalf("rig open work = %d, want 1", len(rehomed))
	}
	if a := strings.TrimSpace(rehomed[0].Assignee); a != "" {
		t.Fatalf("stranded work assignee = %q, want cleared so a fresh pool member can claim it", a)
	}
}

func TestBuildDesiredState_StrandedPoolInstance_LiveOwnerNotRespawned(t *testing.T) {
	cityPath := t.TempDir()
	rigPath := filepath.Join(cityPath, "fixture")
	if err := os.MkdirAll(rigPath, 0o755); err != nil {
		t.Fatalf("create rig dir: %v", err)
	...
	cityStore := beads.NewMemStore()
	rigStore := beads.NewMemStore()
	// A LIVE pool session owns the work under its instance/session name.
	if _, err := cityStore.Create(beads.Bead{
		ID:     "gm-live",
		Title:  "worker",
		Type:   sessionBeadType,
	...
	// The live session's resume tier may keep exactly one desired session, but no
	// EXTRA cold-pool demand may be minted on top of the live owner.
	if got := dsResult.ScaleCheckCounts[template]; got > 0 {
		t.Fatalf("ScaleCheckCounts[%s] = %d, want 0: a live owner's in-flight work must not mint cold-pool demand; stderr:\n%s", template, got, stderr.String())
	}

	// Re-home half must NOT touch work a live session owns.
	open := snapshot.Open()
	released := releaseOrphanedPoolAssignmentsWhenSnapshotsComplete(cityStore, cfg, cityPath, open, dsResult, map[string]beads.Store{"fixture": rigStore})
	if len(released) != 0 {
		t.Fatalf("releaseOrphanedPoolAssignments released %d beads, want 0: live-owned work must stay assigned", len(released))
	}
}

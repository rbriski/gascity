...
func cmdNudgeDrainWithFormat(args []string, inject bool, hookFormat string, stdout, stderr io.Writer) int {
	...
	if writeErr != nil {
		_ = recordQueuedNudgeFailureWithStore(target.cityPath, nudgeStore, queuedNudgeIDs(items), writeErr, time.Now())
		if inject {
			return 0
		}
...
func deliverSessionNudge(target nudgeTarget, message string, mode nudgeDeliveryMode, jsonOutput bool, stdout, stderr io.Writer) int {
	...
	nudgeStore := resolveNudgesStore(store, target.cfg, target.cityPath, nil)
	return deliverSessionNudgeWithWorker(target, store, nudgeStore, newSessionProvider(), message, mode, jsonOutput, stdout, stderr)
}

func deliverSessionNudgeWithWorker(target nudgeTarget, store, nudgeStore beads.Store, sp runtime.Provider, message string, mode nudgeDeliveryMode, jsonOutput bool, stdout, stderr io.Writer) int {
...
func deliverSessionNudgeWithProvider(target nudgeTarget, sp runtime.Provider, mode nudgeDeliveryMode, stdout, stderr io.Writer) int {
	return deliverSessionNudgeWithWorker(target, nil, nil, sp, "check deploy status", mode, false, stdout, stderr)
}
...
func sendMailNotify(target nudgeTarget, sender string) error {
	...
	nudgeStore := resolveNudgesStore(store, target.cfg, target.cityPath, nil)
	return sendMailNotifyWithWorker(target, store, nudgeStore, newSessionProvider(), sender)
}

func sendMailNotifyWithProvider(target nudgeTarget, sp runtime.Provider) error {
	return sendMailNotifyWithWorker(target, nil, nil, sp, "human")
}
...
func tryDeliverQueuedNudgesByPoller(target nudgeTarget, sessionStore, workStore, nudgeStore beads.Store, sp runtime.Provider, quiescence time.Duration, obs worker.LiveObservation) (bool, error) {
	...
	items, rejected := splitQueuedNudgesForTarget(target, items)
	if len(rejected) > 0 {
		if recErr := recordQueuedNudgeFailureWithStore(target.cityPath, nudgeStore, queuedNudgeIDs(rejected), errNudgeSessionFenceMismatch, time.Now()); recErr != nil {
	...
	handle, err := workerHandleForNudgeTarget(target, workStore, sp)
	if err != nil {
	...
	if err != nil {
		...
		if recErr := recordQueuedNudgeFailureWithStore(target.cityPath, nudgeStore, queuedNudgeIDs(items), err, time.Now()); recErr != nil {
			return false, errors.Join(bookkeepErr, recErr)
		}
		return false, bookkeepErr
	}
...
func claimDueQueuedNudgesMatching(cityPath string, now time.Time, match func(queuedNudge) bool) ([]queuedNudge, error) {
	store := openNudgesClassStore(cityPath)

...
func TestDispatchAllQueuedNudgesNoOpInLegacyMode(t *testing.T) {
	...
	delivered, err := dispatchAllQueuedNudges(dir, cfg, nil, nil, nil, nil, newSessionBeadSnapshot(nil))
	if err != nil {
...
func TestDispatchAllQueuedNudgesEmptyQueue(t *testing.T) {
	...
	dir := t.TempDir()
	delivered, err := dispatchAllQueuedNudges(dir, supervisorCfg(), nil, nil, nil, nil, newSessionBeadSnapshot(nil))
	if err != nil {
		t.Fatalf("dispatchAllQueuedNudges: %v", err)
	}
	if delivered != 0 {
...
func TestDispatchAllQueuedNudgesSkipsNotYetDue(t *testing.T) {
	...
	delivered, err := dispatchAllQueuedNudges(dir, supervisorCfg(), nil, nil, nil, runtime.NewFake(), snapshot)
	if err != nil {
		t.Fatalf("dispatchAllQueuedNudges: %v", err)
	}
	if delivered != 0 {
...
func TestDispatchAllQueuedNudgesDeliversAndAcks(t *testing.T) {
	...
	delivered, err := dispatchAllQueuedNudges(dir, supervisorCfg(), store, store, store, fake, snapshot)
	if err != nil {
		t.Fatalf("dispatchAllQueuedNudges: %v", err)
	}
	if delivered != 1 {
...
func TestDispatchAllQueuedNudgesNilCfg(t *testing.T) {
	...
	delivered, err := dispatchAllQueuedNudges(dir, nil, nil, nil, nil, nil, newSessionBeadSnapshot(nil))
	if err != nil {
		t.Fatalf("dispatchAllQueuedNudges: %v", err)
	}
	if delivered != 0 {

...
- **Remaining = the autonomous provisioner (B2)** + its pull side: a `GET /v0/cities/pending` (cross-org, platform-provisioner auth — distinct from the org-scoped `/complete` shipped here) and the **B2 binary** on identity-v0 that pulls pending cities → provisions beads (beads-web `POST /api/projects`, spike-proven) → mints manifold (B5) → launches the Model-B controller (spike-proven) → `POST /complete`. Then **B3** its Deploy/ESO/netpols, and **B1-full** the async create→`pending` + signed-intent hardening for the prod corp-public↔identity-v0 topology. B2 is a new cross-cluster service with its own auth design — the load-bearing remaining unit.

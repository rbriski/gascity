...
func TestCodexHooksDriftCheckPassesCurrentHooks(t *testing.T) {
  ...
  "hooks": {
    "SessionStart": [{
      "hooks": [{
        "type": "command",
        "command": "export PATH=\"$HOME/go/bin:$HOME/.local/bin:$PATH\" && _gcbd_skew=$(bd doctor 2>&1); case \"$_gcbd_skew\" in *\"schema version mismatch\"*|*\"Unable to open database\"*) printf '%%s\\n' \"$_gcbd_skew\" >&2; exit 1 ;; esac; GC_MANAGED_SESSION_HOOK=1 GC_HOOK_EVENT_NAME=SessionStart gc --city %s prime --hook --hook-format codex"
      }]
    }],
    "PreCompact": [{
      "hooks": [{

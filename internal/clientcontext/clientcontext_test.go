		...
		Contexts: []Context{
			{
				...
				GrantCommand: "gc-write-mint --key ~/.gc/keys/maintainer.ed25519",
			},
			{
				Name:              "hosted",
				URL:               "https://works.gascity.com/city-api",
				City:              "acme",
				CredentialCommand: "eia-helper token --audience gc-city",
				CAFile:            "/etc/ssl/gc/hosted-ca.pem",
			},
		},
	}
}
...
func TestLookup(t *testing.T) {
	f := sampleFile()
	if c, ok := f.Lookup("hosted"); !ok || c.URL != "https://works.gascity.com/city-api" {
		t.Errorf("Lookup(hosted) = %+v, %v", c, ok)
	}

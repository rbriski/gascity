	...
	for _, cand := range candidates {
		...
		if !result.Due {
			...
			if a.Trigger == "event" && result.Cursor > eventCursor {
				...
				run, err := front.CreateCursorCheckpoint(scoped, orders.EventCursor(result.Cursor), "event trigger cursor advanced past controller bookkeeping events")
				if err != nil {
				...
				if a.On == events.BeadCreated || a.On == events.BeadClosed {
					post, postErr := checkOrderTriggerBounded(ctx, a, now, lastRunFn, m.ep, func(string) uint64 { return result.Cursor }, triggerOpts)
					switch {
					case postErr != nil:
					...
					case !post.Due && post.Cursor > result.Cursor:
						if err := front.SetCursor(run.ID, scoped, orders.EventCursor(post.Cursor)); err != nil {

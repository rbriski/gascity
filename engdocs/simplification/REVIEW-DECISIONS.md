...
- **Actions:** QUEUED — execute at end (see Action queue › A13).
...
- **Actions:** RELAUNCHED — first run stalled (Fable spec agent tripped the 180s watchdog on the hard byte-for-byte enumeration). Relaunched on the **hardened pipeline** (`graduate-hard.js`: spec writes incrementally, skeleton → section-by-section). Background.

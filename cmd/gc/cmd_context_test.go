...
func TestDoContextAddThenList(t *testing.T) {
	...
	if code := doContextAdd(newTestContext("prod", "https://box:9443", "maintainer-city"), &out, &errb); code != 0 {
	...
	if !strings.Contains(out.String(), "prod") || !strings.Contains(out.String(), "maintainer-city") {
